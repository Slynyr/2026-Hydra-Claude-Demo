...
public class DriveCommands {
    ...
    private static final AngularVelocity     ANGLE_MAX_VELOCITY        = RotationsPerSecond.of(8);
    private static final AngularAcceleration ANGLE_MAX_ACCELERATION    = RotationsPerSecondPerSecond.of(20);
    ...
    /**
     * used for the {@link DriveCommands#crossBump(Drive, Vision, Supplier, Supplier, Time)} command to only start
     * counting the timer once the robot initially leaves the ground
     */
    ...
    public static Command setRotationSpeedHigh() {
        return Commands.run(() -> rotationSpeedModifier = 1.0);
    }

    // Decrease drive translation speed
    public static Command setRotationSpeedLow() {
        return Commands.run(() -> rotationSpeedModifier = 0.5);
    }
    ...
    // Increase drive translation speed
    public static Command setTranslationSpeedHigh() {
        return Commands.run(() -> translationSpeedModifier = 1.0);
    }

    // Decrease drive translation speed
    public static Command setTranslationSpeedLow() {
        return Commands.run(() -> translationSpeedModifier = 0.5);
    }
    ...
    public static Command joystickDrive(
        ...
        return Commands.run(
                () -> {
                    ...
                    ChassisSpeeds speeds = applyRotationDeadband(drive, omegaSupplier, linearVelocity);
    ...
    private static ChassisSpeeds applyRotationDeadband(
            Drive drive, DoubleSupplier omegaSupplier, Translation2d linearVelocity
    ) {
     ...
     * include snapping to an hoodExtension, aiming at a vision target, or controlling absolute rotation with a
     * joystick.
     */
    public static Command joystickDriveAtAngle(
        ...
        ProfiledPIDController angleController =
                new ProfiledPIDController(
                        ...
                        new TrapezoidProfile.Constraints(
                                ANGLE_MAX_VELOCITY.in(RotationsPerSecond),
                                ANGLE_MAX_ACCELERATION.in(RotationsPerSecondPerSecond)));
        ...
        return Commands.run(
                () -> {
                    ...
                    Angle difference = MathUtils.rotationDifference(
                            drive.getRotation(),
                    ...
                    Logger.recordOutput("AutoAlign/MaxVelocity", ANGLE_MAX_VELOCITY);
                    Logger.recordOutput("AutoAlign/MaxAcceleration", ANGLE_MAX_ACCELERATION);
                    Logger.recordOutput("AutoAlign/AngleToAlignment", difference);

//              TODO: TUNE THIS PERCENTAGE TO BE ABLE TO LAUNCH FASTER
//              if (MathUtil.isNear(Degrees.of(drive.getRotation().getDegrees()), Degrees.of(rotationSupplier.get()
//              .getDegrees()), 1.5))
                    if (MathUtils.isAngleNear(
                            drive.getRotation(), rotationSupplier.get(),
                            Degrees.of(1.5)))
                        isAligned = true;
    ...
    public static Command alignToPoint(Drive drive, Supplier<Pose2d> target, Supplier<LinearVelocity> maxVelocity,
                                       Supplier<LinearAcceleration> maxAcceleration) {
        return alignToPoint(
                drive, target, maxVelocity, maxAcceleration, kAutoAlign.TRANSLATION_TOLERANCE,
                kAutoAlign.ROTATION_TOLERANCE, kAutoAlign.VELOCITY_TOLERANCE);
    }

    @SuppressWarnings("resource")
    public static Command alignToPoint(Drive drive, Supplier<Pose2d> target, Supplier<LinearVelocity> maxVelocity,
                                       Supplier<LinearAcceleration> maxAcceleration, Distance translationTolerance,
                                       Angle rotationTolerance, LinearVelocity velocityTolerance) {
        ...
        return Commands.sequence(
                ...
                Commands.run(
                        () -> {
                            ...
                            translationController.setContraints(
                                    maxVelocity.get().in(MetersPerSecond),
                                    maxAcceleration.get().in(MetersPerSecondPerSecond));
                            ...
                            drive.runVelocity(
                                    ChassisSpeeds.fromFieldRelativeSpeeds(speedX, speedY, omega, drive.getRotation()));
                    ...
                    Distance distance = Meters.of(
                            Math.hypot(robotPose.getX() - targetPose.getX(), robotPose.getY() - targetPose.getY()));

                    ChassisSpeeds speeds = drive.getChassisSpeeds();
                    LinearVelocity robotSpeed = MetersPerSecond.of(
        ...
        return Commands.sequence(
                ...
                Commands.run(
                        () -> {
                            ...
                            double omega =
                                    headingController.calculate(
                                            robotRotation.getRadians(), targetRotation.getRadians());
    ...
    public static Command crossBump(Drive drive, Vision vision, Supplier<Rotation2d> targetHeading,
                                    Supplier<LinearVelocity> speed, Time timeout) {
        //  -----------------SIM----------------
        ...
        return Commands.sequence(
                ...
                Commands.run(
                        ...
                        .until(() -> {
                            ...
                            if (drive.getTilt().gt(Degrees.of(2.0)) &&
                                lastTime.get() == -1) { // should be much lower on real robot
                                //     lastTime.set(-2);
                                // }
    ...
    public static Rotation2d getRotationToPassingPosition(Drive drive, BooleanSupplier isRightHalf) {
        return getRotation2d(
                drive, isRightHalf.getAsBoolean()
